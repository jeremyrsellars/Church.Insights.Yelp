...
review_counts <- read_json_data("review-counts", prefix="church-reviews/")
names(review_counts) <- c("date","count")
review_counts$Date <- as.Date(review_counts$date, "%Y-%m-%d")
review_counts$Month <- month(review_counts$Date)
```
...
#+ facet_grid(. ~ Month, margins=TRUE)

ggplot(review_counts, aes(x=Date, y=count)) + 
  geom_histogram(weight=review_counts$count, stat="bin")
#+ facet_grid(. ~ Month, margins=TRUE)
```
